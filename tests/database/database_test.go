...
func TestOpenCreatesMigratesAndClosesOnDiskDatabase(t *testing.T) {
	...
	if got := openTestQueryInt64(t, db, "PRAGMA user_version"); got != 3 {
...
func TestOpenRejectsNewerSchemaVersionAndReturnsNoDatabase(t *testing.T) {
	...
	if _, err := seed.Exec("PRAGMA user_version = 4"); err != nil {
		seed.Close()
	...
	if got := openTestQueryInt64(t, check, "PRAGMA user_version"); got != 4 {
		t.Fatalf("schema version after failed startup = %d, want 4", got)
	}
}
